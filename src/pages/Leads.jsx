      ...
      <div className="flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0">
          <div className="flex items-center gap-6">
            <h1 className="text-xl font-black text-slate-900 tracking-tight">Growth & Sales Engine</h1>
          ...
          <div className="flex items-center gap-2 text-xs font-medium text-slate-400">
             {activeModule === 'CAMPAIGNS' ? (
               <>Running 4 Active Campaigns <ArrowRightIcon className="h-3 w-3" /> Generating Leads</>
             ) : (
               <>Processing 482 Leads <ArrowRightIcon className="h-3 w-3" /> Closing Deals</>
             )}
          </div>
        </header>
        ...
        <div className="flex-1 overflow-hidden relative">
          {activeModule === "CAMPAIGNS" ? <CampaignManager /> : <PipelineManager />}
        </div>
      </div>
    </div>
  );

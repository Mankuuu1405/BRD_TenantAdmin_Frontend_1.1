...
import { thirdPartyAPI, locationAPI } from "../services/thirdPartyUsersService.js";
...
export default function ThirdPartyUsers() {
  ...
  useEffect(() => {
    ...
    if (country) {
      locationAPI.getStates(country).then(data => {
        setStates(data);
        if (!editing) { // Reset lower levels only if creating new
  ...
  const validate = (data) => {
    ...
    // Password check only for creation
    if (creating) {
  ...
  const handleSave = async () => {
    ...
    try {
      const newUser = await thirdPartyAPI.create(form);
      setItems(prev => [...prev, newUser]);
      setCreating(false);
      setForm(initialForm);
      setValidationError("");
    } catch (e) {
      setValidationError("Failed to create user on server.");
    }
  };

  const handleUpdate = async () => {
    try {
      const updated = await thirdPartyAPI.update(editing.id, editing);
  ...
  const handleDelete = async () => {
    try {
      await thirdPartyAPI.delete(deleting.id);
                ...
                <tr key={u.id} className="hover:bg-blue-50/30 transition-colors group">
                  <td className="px-8 py-6">
                    ...
                    <div className="text-xs font-bold text-blue-600 mt-1 flex items-center gap-1">
                      <Icons.User /> User ID: #{u.id}
                    </div>
                  </td>
                  <td className="px-8 py-6">
                    <div className="font-bold text-slate-700">{u.organizationName}</div>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-2 items-start">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide border ${u.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                        {u.status}
                      </span>
            ...
            <div className="p-8 overflow-y-auto space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormGroup label="PAN Number">
                  <InputBox value={form.pan} onChange={e => setForm({ ...form, pan: e.target.value })} placeholder="PAN Card Number" icon={<Icons.Identification />} />
                </FormGroup>
              </div>

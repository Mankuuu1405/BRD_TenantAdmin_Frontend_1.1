...
const StatCard = ({ title, value, icon, colorClass }) => (
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-xl flex items-center gap-4">
...
export default function Users() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const tenantUsers = allUsers.filter(u => 
        String(u.tenant_id) === String(currentTenant) || 
        String(u.tenant) === String(currentTenant)
      );
  ...
  const mapFormToPayload = (formData) => {
    const payload = {
        ...
        role_id: formData.role_id || null, // ✅ Send null for empty string
  ...
  const validateForm = (data, isEdit = false) => {
    ...
    if (!isEdit && !data.password && !isSupervisorRole(data.role_type)) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      console.error("Creation failed", e.response?.data || e);
    ...
    <div className="p-12 max-w-[1800px] mx-auto min-h-screen bg-slate-50 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        <div>
          ...
          <p className="text-slate-500 text-lg font-medium">Manage employees, roles, and access permissions for your tenant.</p>
        </div>
        <div className="flex gap-3">
             <Button variant="ghost" onClick={fetchInitialData} icon={<Icons.ArrowPath />}>Refresh</Button>
             <Button variant="secondary" onClick={() => window.print()} icon={<Icons.Building />}>Export List</Button>
             <Button onClick={() => { setCreating(true); setForm(initialForm); setValidationError(""); }} icon={<Icons.Plus />}>Add New User</Button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] shadow-xl border border-slate-200 overflow-hidden mb-8">
        <div className="p-6 border-b border-slate-100 bg-white flex flex-col md:flex-row gap-4 items-center justify-between">
           <div className="relative flex-1 w-full max-w-lg">
             <span className="absolute left-5 top-3.5 text-slate-400"><Icons.Search /></span>
             <input className="w-full pl-14 pr-6 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400" placeholder="Search by name, email or role..." value={search} onChange={e => setSearch(e.target.value)} />
           </div>
           <div className="flex items-center gap-3 w-full md:w-auto">
                <span className="text-xs font-extrabold text-slate-400 uppercase tracking-widest whitespace-nowrap">Filter Role:</span>
                <div className="min-w-[180px]">
                    <SelectBox options={[{value: 'ALL', label: 'All Roles'}, ...ROLE_OPTIONS]} value={roleFilter} onChange={e => setRoleFilter(e.target.value)} />
                </div>
           </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/80 text-slate-500 font-extrabold uppercase tracking-widest text-[11px] border-b border-slate-200">
              <tr>
                ...
                <th className="px-8 py-5">Supervisor Info</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                filteredItems.map((u) => (
                    ...
                    <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-slate-200 text-slate-600 flex items-center justify-center font-bold text-sm border-2 border-white shadow-sm">{u.first_name?.[0]}{u.last_name?.[0]}</div>
                            <div>
                                <div className="font-bold text-slate-800 text-sm">{u.first_name} {u.last_name}</div>
                                <div className="text-[10px] font-bold text-indigo-500 mt-0.5">Tenant: {u.tenant_id || u.tenant}</div>
                            </div>
                        </div>
                    </td>
                    <td className="px-8 py-5">
                        <div className="flex flex-col gap-1">
                            <span className="inline-flex w-fit items-center px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wide bg-slate-100 text-slate-600 border border-slate-200">{u.role || u.role_type}</span>
                            <span className="text-[10px] text-slate-400 font-bold tracking-wider">{u.role_id && roleMap[u.role_id] ? `Role: ${roleMap[u.role_id]}` : `RID: ${u.role_id || "N/A"}`}</span>
                        </div>
                    </td>
                    <td className="px-8 py-5">
                        <div className="flex flex-col gap-1.5">
                            ...
                            <div className="flex items-center gap-2 text-xs font-medium text-slate-500"><span className="text-slate-400"><Icons.Phone/></span> {u.phone || u.mobile_no || "-"}</div>
                        </div>
                    </td>
                    <td className="px-8 py-5">
                        {isSupervisorRole(u.role || u.role_type) ? (
                            <div className="bg-purple-50 p-2 rounded-lg border border-purple-100 max-w-[200px]">
                                <div className="text-[10px] uppercase tracking-widest font-extrabold text-purple-400 mb-1">Supervisor Details</div>
                                <div className="text-xs font-bold text-purple-900">{u.supervisor_name}</div>
                                <div className="text-[10px] text-purple-600 truncate" title={u.supervisor_email}>{u.supervisor_email}</div>
                                <div className="text-[10px] text-purple-600">{u.supervisor_mobile}</div>
                            </div>
                        ) : <span className="text-xs text-slate-300 italic">N/A</span>}
                    </td>
                    <td className="px-8 py-5">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide border ${u.is_active || u.status === 'Active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                    ...
                    <td className="px-8 py-5 text-right">
                        ...
                        <button onClick={() => { setEditing({...u, mobile_no: u.phone || u.mobile_no, role_type: u.role || u.role_type, status: u.is_active ? "Active" : "Inactive", password: ""}); setValidationError(""); }} className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition"><Icons.Edit /></button>
                ...
                <tr><td colSpan={6} className="p-16 text-center text-slate-400 font-bold uppercase tracking-widest text-xs">No users match your search</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-8 overflow-y-auto space-y-8">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <FormGroup label="Role ID (Permission Group)" required>
                    <SelectBox options={roles.length > 0 ? roles.map(r => ({ value: r.id, label: r.name || r.role_name })) : []} value={form.role_id} onChange={e => setForm({...form, role_id: e.target.value})} placeholder="Select Role ID" icon={<Icons.Key />} />
                  </FormGroup>
                  {!isSupervisorRole(form.role_type) && (
                      <FormGroup label="Login Password" required>
                        <div className="flex gap-2">
                            <InputBox type="text" value={form.password} onChange={e => setForm({...form, password: e.target.value})} placeholder="Enter secure password" icon={<Icons.Lock />} />
                            <button type="button" onClick={() => setForm({...form, password: generatePassword()})} className="px-3 py-2 bg-slate-100 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-200">Generate</button>
                        </div>
                      </FormGroup>
                  )}
               ...
               {isSupervisorRole(form.role_type) && (
                   <div className="bg-purple-50 p-6 rounded-2xl border border-purple-100 animate-fade-in">
                       ...
                       <div className="grid grid-cols-1 gap-4">
                           <FormGroup label="Supervisor Name" required><InputBox value={form.supervisor_name} onChange={e => setForm({...form, supervisor_name: e.target.value})} placeholder="Enter Supervisor Name" icon={<Icons.User />} className="bg-white" /></FormGroup>
                           <div className="grid grid-cols-2 gap-4">
                                <FormGroup label="Supervisor Email" required><InputBox type="email" value={form.supervisor_email} onChange={e => setForm({...form, supervisor_email: e.target.value})} placeholder="supervisor@email.com" icon={<Icons.Mail />} className="bg-white" /></FormGroup>
                                <FormGroup label="Supervisor Mobile" required><InputBox type="tel" value={form.supervisor_mobile} onChange={e => setForm({...form, supervisor_mobile: e.target.value})} placeholder="Mobile No" icon={<Icons.Phone />} className="bg-white" /></FormGroup>
                           </div>
                       </div>
                   </div>
               )}
            ...
            <div className="p-6 bg-slate-50 border-t border-slate-200 flex justify-between items-center">
               <div className="text-xs font-bold text-rose-500">{validationError}</div>
               <div className="flex gap-3">
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-slate-50 px-8 py-6 border-b border-slate-200 flex justify-between items-center">
               <div className="flex items-center gap-4">
                 <div className="w-12 h-12 rounded-2xl bg-orange-100 text-orange-600 flex items-center justify-center"><Icons.Edit /></div>
                 <div><h2 className="text-xl font-extrabold text-slate-900">Edit User Details</h2><p className="text-xs text-slate-500 font-bold uppercase tracking-wider">ID: {editing.id} • {editing.first_name} {editing.last_name}</p></div>
               </div>
               <button onClick={() => setEditing(null)} className="text-slate-400 hover:text-rose-500 transition"><Icons.Close /></button>
            </div>
            <div className="p-8 overflow-y-auto space-y-8">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <FormGroup label="Role ID" required><SelectBox options={roles.length > 0 ? roles.map(r => ({ value: r.id, label: r.name || r.role_name })) : []} value={editing.role_id} onChange={e => setEditing({...editing, role_id: e.target.value})} icon={<Icons.Key />} /></FormGroup>
                  {!isSupervisorRole(editing.role_type) && (
                      <FormGroup label="Reset Password (Optional)">
                        <div className="flex gap-2">
                            ...
                            <button type="button" onClick={() => setEditing({...editing, password: generatePassword()})} className="px-3 py-2 bg-slate-100 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-200">Gen</button>
                        </div>
                      </FormGroup>
                  )}
               ...
               {isSupervisorRole(editing.role_type) && (
                   <div className="bg-purple-50 p-6 rounded-2xl border border-purple-100">
                       ...
                       <div className="space-y-4">
                           <FormGroup label="Supervisor Name" required><InputBox value={editing.supervisor_name} onChange={e => setEditing({...editing, supervisor_name: e.target.value})} className="bg-white" /></FormGroup>
                           <div className="grid grid-cols-2 gap-4">
                                <FormGroup label="Email" required><InputBox value={editing.supervisor_email} onChange={e => setEditing({...editing, supervisor_email: e.target.value})} className="bg-white" /></FormGroup>
                                <FormGroup label="Mobile" required><InputBox value={editing.supervisor_mobile} onChange={e => setEditing({...editing, supervisor_mobile: e.target.value})} className="bg-white" /></FormGroup>
                           </div>
                       </div>
                   </div>
               )}
      ...
      {viewing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
             <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl p-8 relative">
                 <button onClick={() => setViewing(null)} className="absolute top-6 right-6 text-slate-300 hover:text-slate-600"><Icons.Close /></button>
                 <div className="flex flex-col items-center mb-8">
                     <div className="w-20 h-20 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center text-2xl font-black mb-4">{viewing.first_name?.[0]}{viewing.last_name?.[0]}</div>
                     <h2 className="text-2xl font-black text-slate-900">{viewing.first_name} {viewing.last_name}</h2>
                     <span className="bg-indigo-50 text-indigo-600 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider mt-2">{viewing.role || viewing.role_type}</span>
                 </div>
                 <div className="space-y-4 bg-slate-50 rounded-2xl p-6 border border-slate-100">
                     <div className="flex justify-between border-b border-slate-200 pb-2"><span className="text-xs font-bold text-slate-400 uppercase">Email</span><span className="text-sm font-bold text-slate-700">{viewing.email}</span></div>
                     <div className="flex justify-between border-b border-slate-200 pb-2"><span className="text-xs font-bold text-slate-400 uppercase">Mobile</span><span className="text-sm font-bold text-slate-700">{viewing.phone || viewing.mobile_no}</span></div>
                     <div className="flex justify-between border-b border-slate-200 pb-2"><span className="text-xs font-bold text-slate-400 uppercase">Status</span><span className={`text-sm font-bold ${viewing.is_active || viewing.status === 'Active' ? 'text-emerald-600' : 'text-slate-500'}`}>{viewing.is_active ? "Active" : "Inactive"}</span></div>
                     <div className="flex justify-between"><span className="text-xs font-bold text-slate-400 uppercase">Role ID</span><span className="text-sm font-bold text-slate-700">{viewing.role_id || "N/A"}</span></div>
                 </div>
                 {isSupervisorRole(viewing.role || viewing.role_type) && (
                     <div className="mt-4 bg-purple-50 rounded-2xl p-6 border border-purple-100">
                         ...
                         <p className="text-sm text-purple-800 mb-1"><strong>Email:</strong> {viewing.supervisor_email}</p>
      ...
      {deleting && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full text-center border border-slate-200">
            <div className="w-16 h-16 bg-rose-50 text-rose-500 rounded-full flex items-center justify-center mx-auto mb-4 text-2xl"><Icons.Trash /></div>
            <h3 className="text-xl font-extrabold text-slate-800">Delete User Account</h3>
            <p className="text-slate-500 mt-2 mb-6 text-sm">Are you sure you want to remove <span className="font-bold text-slate-800">{deleting.first_name} {deleting.last_name}</span>?</p>
            <div className="flex justify-center gap-3"><Button variant="secondary" onClick={() => setDeleting(null)}>Cancel</Button><Button variant="danger" onClick={handleDelete}>Confirm Delete</Button></div>
          </div>
        </div>
      )}
